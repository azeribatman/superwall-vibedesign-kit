...
import os
# Any app the authenticated user has access to works for listing GLOBAL templates.
# Override via env: APP_ID=12345 python3 scripts/pull_templates.py
...
def main():
    if not APP_ID:
        ...
        print("  e.g.   APP_ID=12345 python3 scripts/pull_templates.py")
        return 2
    t0 = time.time()
    client = SuperwallClient()
    print(f"listing templates (app {APP_ID})...")
